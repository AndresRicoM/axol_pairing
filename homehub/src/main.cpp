...
void loop()
{
  ...
  if (!digitalRead(b))
  {
    ...
    String titulo = "Agua Total";
    String porcentaje = "92%";

    int16_t x1, y1;
    uint16_t w, h;

    // ---- CENTRAR TITULO ARRIBA ----
    display.getTextBounds(titulo, 0, 0, &x1, &y1, &w, &h);
    int xTitulo = (display.width() - w) / 2;
    display.setCursor(xTitulo, 0);
    display.print(titulo);

    // ---- CENTRAR PORCENTAJE ABAJO ----
    display.getTextBounds(porcentaje, 0, 0, &x1, &y1, &w, &h);
    int xPorcentaje = (display.width() - w) / 2;

    int yPorcentaje = display.height() - h;   // lo manda hasta abajo
    display.setCursor(xPorcentaje, yPorcentaje);
    display.print(porcentaje);

    // ---- DIBUJAR TANQUE EN MEDIO ----
    draw.draw_tank92();
